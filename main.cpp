...
void insertIntoTree(Btree<int> &tester);
void removeFromTree(Btree<int> &tester);
void sortAndPruneHelper();
bool testTreeCompletion(Btree<int>* tester);
void print(Btree<int>* tester);
...
int main(int argc, char *argv[]) {
    ...
    assert(tester.weigth() == helper.size() && "Something is wrong with the insert or weight method");
...
bool testTreeCompletion(Btree<int>* tester) {
    int i = 0;
    for (InOrderIterator<int> it = tester->begin(); it != tester->end(); ++it) {
        if (*it != helper.at(i)) {
            return false;
        }
        i++;
